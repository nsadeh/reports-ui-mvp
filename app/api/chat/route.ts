...
function buildSystemPrompt(reportId: string): string {
  ...
  const scope = meta?.scope_value ?? "this topic area";
  const title = meta?.title ?? "this report";

  const prefix = `You are an analyst at Inflection Labs specializing in pipeline and market access intelligence. You have access to the full report below.
...
export async function POST(req: Request) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    ...
    const answer =
      ...
      "I can answer questions about the pipeline and market access landscape covered in this report. Try asking about specific drugs, clinical trial updates, regulatory decisions, or funding events.";
  ...
  const reportDirName = getReportDirName(reportId) ?? reportId;
  ...
  const result = streamText({
    ...
    tools: {
      search_corpus: tool<{ query: string }, string>({
        ...
        execute: async ({ query }) => searchCorpus(query, reportDirName),
      }),
      get_document: tool<{ filename: string }, string>({
        ...
        execute: async ({ filename }) => getDocument(filename, reportDirName),
      }),
      list_documents: tool<Record<string, never>, string>({
        ...
        inputSchema: z.object({}),
        execute: async () => listDocuments(reportDirName),
      }),
    },
  });
